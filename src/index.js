...
io.on('connection', (socket) => {
  socket.on('join', ({ username, room }, callbackMessage) => {
  ...
  socket.on('sendMessage', (newMessage, callbackProfanity) => {
    const user = getUser(socket.id);
    const filter = new Filter();

    if (filter.isProfane(newMessage)) {
      return callbackProfanity('Profanity is not allowed!');
    }
